...
from models.ResnetonCifar import resnet56
...
if __name__ == '__main__':
    ...
    print('开始训练', str(datetime.datetime.now()))
    print(net_glob)
    net_glob.train()
    ...
    for iter in range(args.epochs):
        ...
        print('聚合开始时间', str(datetime.datetime.now()))
        agg_start = time.time()
        w_glob = FedAvg(w_locals)
        agg_end = time.time()
        agg_time = (agg_end-agg_start)
        # %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        print('聚合结束时间', str(datetime.datetime.now()))
